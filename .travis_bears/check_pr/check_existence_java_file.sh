...
PATCHED_COMMIT_STATS=$(git show --stat -n 1 --grep="Human patch" --format=format:%f -- '*.java')
...
if [ "$CASE" == "passing_passing" ]; then
    TEST_COMMIT_STATS=$(git show --stat -n 1 --grep="Changes in the tests" --format=format:%f -- '*.java')
